...
# Python dict obj cannot handle over 80mil ish rows, thus I had to use SQL and 3 separate json files
# Used https://www.statskingdom.com/combinations-calculator.html to find the total combinatons, then split total data into three loads
TG = TableGenerator()
# TG.GenerateLarge(numDigits, lenChains, 61, 0, 40000000)
# TG.GenerateLarge(numDigits, lenChains, 62, 40000001, 80000000)
# TG.GenerateLarge(numDigits, lenChains, 63, 80000001, 113379904)

# Load SQL DB with json data, again uncomment only if numDigits changes
SQ = SQLUtilities()
# SQ.LoadDb(61, tableName)
# SQ.LoadDb(62, tableName)
# SQ.LoadDb(63, tableName)
...
def checkHashes(testHash, numDigits, lenChains, checkedHashes, collisions):
    ...
    while (solvedInput[0] == False):
        solvedInput = CT.CheckSQLTable(
            testHash, numDigits, checkedHashes, lenChains, collisions, tableName)
